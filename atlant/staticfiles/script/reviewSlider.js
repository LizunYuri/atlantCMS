document.addEventListener("DOMContentLoaded", function() {
    var reviews = document.querySelectorAll('.review-card'); // Все отзывы
    var currentStartIndex = 0;
    var reviewsPerPage = 2; // Изначально 2 отзыва на страницу

    function updateReviewsPerPage() {
        if (window.innerWidth < 1200) {
            reviewsPerPage = 1; // Если ширина экрана меньше 768px, показываем 1 отзыв
        } else {
            reviewsPerPage = 2; // Если ширина экрана больше или равна 768px, показываем 2 отзыва
        }
    }
